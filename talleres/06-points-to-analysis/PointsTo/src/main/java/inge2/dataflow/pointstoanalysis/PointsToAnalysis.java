...
public class PointsToAnalysis extends ForwardFlowAnalysis<Unit, PointsToGraph> {
    ...
    public boolean mayAlias(String leftVariableName, String rightVariableName) {
        Set<Node> leftVariableNodes = new HashSet<>(this.lastPointsToGraph.getNodesForVariable(leftVariableName));
        Set<Node> rightVariableNodes = this.lastPointsToGraph.getNodesForVariable(rightVariableName);
        leftVariableNodes.retainAll(rightVariableNodes);

        if (!leftVariableNodes.isEmpty()) { // si la interseccion no es vacia (i.e. hay coincidencia)
            return true;
        }
        return false;
    }
    ...
    public boolean mayAlias(String leftVariableName, String fieldName, String rightVariableName) {
        Set<Node> leftVariableNodes = this.lastPointsToGraph.getNodesForVariable(leftVariableName);
        Set<Node> rightVariableNodes = this.lastPointsToGraph.getNodesForVariable(rightVariableName);
        Set<Node> reachablesNodesByLeftVariable = new HashSet<>();

        for(Node leftVariableNode : leftVariableNodes) {
            Set<Node> reachableNodesOfLeftNode = this.lastPointsToGraph.getReachableNodesByField(leftVariableNode, fieldName); // todo n2
            reachablesNodesByLeftVariable.addAll(reachableNodesOfLeftNode);
        }
        reachablesNodesByLeftVariable.retainAll(rightVariableNodes);
        if (!reachablesNodesByLeftVariable.isEmpty()) { // si la interseccion no es vacia (i.e. hay coincidencia)
            return true;
        }
        return false;
    }
